...
backup() {
  ...
  SERVERS_ROOT_VAL="$(grep -E '^SERVERS_ROOT=' "${APP_DIR}/.env" | cut -d= -f2- || true)"
  BACKUPS_ROOT_VAL="$(grep -E '^BACKUPS_ROOT=' "${APP_DIR}/.env" | cut -d= -f2- || true)"
  {
  ...
  if dc ps postgres 2>/dev/null | grep -q "Up\|running"; then
    local pg_user pg_db
    pg_user="$(grep -E '^POSTGRES_USER=' "${APP_DIR}/.env" | cut -d= -f2- || echo platform)"
    pg_db="$(grep -E '^POSTGRES_DB=' "${APP_DIR}/.env" | cut -d= -f2- || echo platform)"
