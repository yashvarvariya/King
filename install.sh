...
system_checks() {
  ...
  if [ "$DISK_AVAIL_GB" -gt 0 ] && [ "$DISK_AVAIL_GB" -lt 5 ]; then
...
# ============================================================================
# Step 4: Interactive configuration + .env generation
# ============================================================================
...
# ============================================================================
# Step 8: Start services (+ first-cert bootstrap dance)
# ============================================================================
start_services() {
...
health_check() {
  ...
  if [ "$svc_ok" -eq 0 ] || [ "$api_ok" -eq 0 ]; then
    HEALTH_OK=0
  fi
...
# ============================================================================
# Step 12: Management command + summary
# ============================================================================
...
print_summary() {
  ...
  if [ "$HEALTH_OK" -eq 0 ]; then
    c_yellow "One or more health checks did not pass yet. This is often just services"
    c_yellow "still warming up. Check again in a minute with: panel status"
  else
