...
interface CreateContainerOpts {
  ...
  // Optional Runtime Manager catalog override (see runtimes/runtimes.service.ts).
  // When a server has a runtimeVersionId assigned, this is that version's
  // Docker image tag and takes priority over the hardcoded RUNTIME_IMAGE
  // map below. Servers created before the Runtime Manager existed never
  // set this, so they keep resolving through the legacy map unchanged.
  image?: string;
}
...
export class DockerService {
  ...
  async createContainer(opts: CreateContainerOpts): Promise<string> {
    const image = opts.image || RUNTIME_IMAGE[opts.runtime];
